      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
          ...
          <span className="text-sm font-semibold text-gray-900">
            {profile?.userId.city || 'N/A'}{profile?.userId.town && profile?.userId.city ? `, ${profile.userId.town}` : profile?.userId.town || ''}
          </span>
        </div>
            ...
            <AnimatePresence>
              {sortedUsers.map((user, idx) => {
                console.log("user",user);
                    console.log("sssssss",profiles); 

const profil = profile.find(p => p?.userId?._id?.toString() === user._id?.toString());
    console.log("userchecking", user._id, profil || "No profile found"); 
